...
% 
% WS2_1=transtows_resource(structure3_1);
% APL_index2(1)=APL2(WS2_1);
% 
% WS2_2=transtows_resource(structure3_2);
% APL_index2(2)=APL2(WS2_2);
% 
% WS2_3=transtows_resource(structure3_3);
% APL_index2(3)=APL2(WS2_3);
% 
% WS2_4=transtows_resource(structure3_4);
% APL_index2(4)=APL2(WS2_4);
% 
% WS2_5=transtows_resource(structure3_5);
...
N=20000;
Nwarm=1500;
arrival=zeros(size(D3,1),upperbound_arrival);
...
num_demand=max(arc3_1(:,2));
num_staff=max(arc3_1(:,1));
...
for i=1:num_demand
    arrival(i,:)=gamrnd(alpha_arrival,1/D3(i)/alpha_arrival,1,upperbound_arrival);
...
while(sum(arrival_served2(:,end)>0)<=N+Nwarm) %arrival_flag是目前到达的第x个
    structure_this=eval(['structure3_',num2str(sflag)]);
...
filen=['d3_',int2str(cva(iiii)),int2str(cvs(jjjj)),int2str(us(kkkk))];
save(filen);
        end
    end
end